...
static inline char mstream_read_char(struct mstream* ms)
{
    return ((const char*)ms->address)[ms->idx++];
}
static inline uint16_t mstream_read_lu16(struct mstream* ms)
{
    uint16_t value;
...
static inline uint32_t mstream_read_lu32(struct mstream* ms)
{
    uint32_t value;
...
static inline uint64_t mstream_read_lu64(struct mstream* ms)
{
    uint64_t value;
...
static inline float mstream_read_lf32(struct mstream* ms)
{
    float value;
    memcpy(&value, (const char*)ms->address + ms->idx, 4);
    ms->idx += 4;
...
static inline const void* mstream_read(struct mstream* ms, int len)
{
    const void* data = (const char*)ms->address + ms->idx;
    ms->idx += len;
    return data;
}
static inline const void* mstream_ptr(struct mstream* ms)
{
    return (const char*)ms->address + ms->idx;
}
...
mstream_read_string_until_delim(
        struct mstream* ms, char delim, struct string* str);

int mstream_read_string_until_condition(
        struct mstream* ms, int (*cond)(char), struct string* str);
