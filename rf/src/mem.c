...
static RF_THREADLOCAL uintptr_t g_allocations = 0;
static RF_THREADLOCAL uintptr_t d_deallocations = 0;
static RF_THREADLOCAL uintptr_t g_ignore_hm_malloc = 0;
RF_THREADLOCAL uintptr_t g_bytes_in_use = 0;
RF_THREADLOCAL uintptr_t g_bytes_in_use_peak = 0;
static RF_THREADLOCAL struct cs_hashmap g_report;

typedef struct report_info_t
{
    void* location;
    uintptr_t size;
#   if defined(RF_MEMORY_BACKTRACE)
    int backtrace_size;
    char** backtrace;
#   endif
} report_info_t;
...
/* ------------------------------------------------------------------------- */
void*
rf_realloc(void* p, uintptr_t new_size)
{
    void* old_p = p;
...
#else /* RF_MEMORY_DEBUGGING */

int rf_mem_threadlocal_init(void)         { return 0; }
uintptr_t rf_mem_threadlocal_deinit(void) { return 0; }
uintptr_t rf_mem_get_num_allocs(void)     { return 0; }
uintptr_t rf_mem_get_memory_usage(void)   { return 0; }

#endif /* RF_MEMORY_DEBUGGING */

/* ------------------------------------------------------------------------- */
void
rf_mem_mutated_string_and_hex_dump(const void* data, uintptr_t length_in_bytes)
{
    char* dump;
    uintptr_t i;
