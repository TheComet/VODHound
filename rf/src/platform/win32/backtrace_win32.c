...
#define RF_BACKTRACE_FUNC_LEN 1024
...
rf_backtrace_get(int* size)
{
    ...
    char** current_ptr;
    char* current_str;
    void* stack[RF_BACKTRACE_SIZE];
    char sym_buf[sizeof(SYMBOL_INFO) + (RF_BACKTRACE_FUNC_LEN - 1) * sizeof(TCHAR)];
    WORD frames_traced = CaptureStackBackTrace(0, RF_BACKTRACE_SIZE, stack, NULL);

    result = malloc(
        sizeof(char*) * frames_traced +  /* String table */
        sizeof(char)  * frames_traced * RF_BACKTRACE_FUNC_LEN);
    current_ptr = result;
    current_str = (char*)(result + frames_traced);

    SYMBOL_INFO* sym = (SYMBOL_INFO*)sym_buf;
    sym->MaxNameLen = RF_BACKTRACE_FUNC_LEN;
    ...
    for (int i = 0; i < frames_traced; ++i)
    {
        ...
        *current_ptr = current_str;
        ...
        if (SymGetLineFromAddr64(hProcess, address, &displacement, &line) == TRUE)
            current_str += snprintf(current_str, RF_BACKTRACE_FUNC_LEN, "%d: (0x%llx+0x%x) %s:%d", i, sym->Address, displacement, sym->Name, line.LineNumber) + 1;
        else
