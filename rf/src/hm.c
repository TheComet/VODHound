 ...
 */
static rf_hash32
hash_wrapper(const struct rf_hm* hm, const void* data, int len)
{
    rf_hash32 hash = hm->hash(data, len);
    if (hash == RF_HM_SLOT_UNUSED || hash == RF_HM_SLOT_RIP || hash == RF_HM_SLOT_INVALID)
        return 3;
    return hash;
}
...
malloc_and_init_storage(int key_size, int value_size, int table_count)
{
    ...
    storage = rf_malloc((sizeof(rf_hash32) + (size_t)key_size + (size_t)value_size) * (size_t)table_count);
    if (storage == NULL)
        return NULL;

    /* Initialize hash table -- NOTE: Only works if HM_RF_HM_SLOT_UNUSED is 0 */
    memset(storage, 0, (sizeof(rf_hash32) + (size_t)key_size) * (size_t)table_count);
    return storage;
}
...
resize_rehash(struct rf_hm* hm, int new_table_count)
{
    ...
    memcpy(&new_hm, hm, sizeof(struct rf_hm));
    ...
    for (i = 0; i != hm->table_count; ++i)
    {
        if (SLOT(hm, i) == RF_HM_SLOT_UNUSED || SLOT(hm, i) == RF_HM_SLOT_RIP)
            continue;
        if (rf_hm_insert(&new_hm, KEY(hm, i), VALUE(hm, i)) != 0)
        {
            rf_free(new_hm.storage);
            return -1;
        }
    }
...
{
    return rf_hm_create_with_options(
            key_size, value_size,
            RF_HM_MIN_CAPACITY,
            rf_hash32_jenkins_oaat);
}
...
{
    struct rf_hm* hm = rf_malloc(sizeof(*hm));
    if (hm == NULL)
        return NULL;

    rf_hm_init_with_options(hm, key_size, value_size, table_count, hash_func);
    return hm;
}
...
{
    return rf_hm_init_with_options(
...
/* ------------------------------------------------------------------------- */
int
rf_hm_insert(struct rf_hm* hm, const void* key, const void* value)
{
    rf_hash32 hash;
    int pos, i, last_tombstone;

    /* NOTE: Rehashing may change table count, make sure to compute hash after this */
    if (hm->slots_used * 100 / hm->table_count >= RF_HM_REHASH_AT_PERCENT)
        if (resize_rehash(hm, hm->table_count * RF_HM_EXPAND_FACTOR) != 0)
            return -1;

    /* Init values */
    hash = hash_wrapper(hm, key, hm->key_size);
    pos = (int)(hash & (rf_hash32)(hm->table_count - 1));
    i = 0;
    last_tombstone = RF_HM_SLOT_INVALID;

    while (SLOT(hm, pos) != RF_HM_SLOT_UNUSED)
    {
    ...
    if (last_tombstone != RF_HM_SLOT_INVALID)
    {
        pos = last_tombstone;
...
/* ------------------------------------------------------------------------- */
void*
rf_hm_emplace(struct rf_hm* hm, const void* key)
{
    rf_hash32 hash;
    int pos, i, last_tombstone;
...
/* ------------------------------------------------------------------------- */
void*
rf_hm_erase(struct rf_hm* hm, const void* key)
{
    rf_hash32 hash = hash_wrapper(hm, key, hm->key_size);
    int pos = (int)(hash & (rf_hash32)(hm->table_count - 1));
    int i = 0;
    ...
    while (1)
    {
        ...
        {
            if (SLOT(hm, pos) == RF_HM_SLOT_UNUSED)
                return NULL;
        }
