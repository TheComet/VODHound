...
#define VH_BACKTRACE_FUNC_LEN 1024
...
backtrace_get(int* size)
{
    ...
    char** current_ptr;
    char* current_str;
    void* stack[VH_BACKTRACE_SIZE];
    char sym_buf[sizeof(SYMBOL_INFO) + (VH_BACKTRACE_FUNC_LEN - 1) * sizeof(TCHAR)];
    WORD frames_traced = CaptureStackBackTrace(0, VH_BACKTRACE_SIZE, stack, NULL);

    result = malloc(
        sizeof(char*) * frames_traced +  /* String table */
        sizeof(char)  * frames_traced * VH_BACKTRACE_FUNC_LEN);
    current_ptr = result;
    current_str = (char*)(result + frames_traced);

    SYMBOL_INFO* sym = (SYMBOL_INFO*)sym_buf;
    sym->MaxNameLen = VH_BACKTRACE_FUNC_LEN;
